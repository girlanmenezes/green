...
class DB:
    ...
    def get_conta_banco(self, atendimento):
  
        query = f"select cd_conta from ( select cd_conta from ( select reg_fat.cd_reg_fat as cd_conta from dbamv.reg_fat, dbamv.atendime, dbamv.paciente, dbamv.convenio where atendime.cd_atendimento = reg_fat.cd_atendimento and atendime.cd_paciente = paciente.cd_paciente and convenio.cd_convenio = atendime.cd_convenio and atendime.cd_atendimento = '{atendimento}' ) union select cd_conta from ( select itreg_amb.cd_reg_amb as cd_conta from dbamv.itreg_amb, dbamv.atendime, dbamv.paciente, dbamv.convenio where atendime.cd_atendimento = itreg_amb.cd_atendimento and atendime.cd_paciente = paciente.cd_paciente and convenio.cd_convenio = atendime.cd_convenio and atendime.cd_atendimento = '{atendimento}') )"
